...
class BedrockRagOrchestrator:
    ...
    def build(self) -> StateGraph:
        ...
        def node_embed(state: AgentState):
            query = state["query"]
            logs = list(state.get("logs", []))
            existing_vector = state.get("query_vector", [])
            if existing_vector and state.get("_last_embedded_query") == query:
                logs.append(f"Embedding reutilizado para: '{query[:50]}...'")
                return {"logs": logs}
            
            vector, latency = self.embedding.get_embedding(query)
            logs.append(f"Embedding generado en {latency:.2f}ms para: '{query[:50]}...'")
            return {"query_vector": vector, "embedding_latency_ms": latency, "logs": logs, "_last_embedded_query": query}
        ...
        def node_generate(state: AgentState):
            ...
            generation = self.llm.generate(prompt, system_prompt=SYSTEM_PROMPT_GENERATION_BEDROCK)
            ...
            if not documents:
                ...
                logs.append(f"Respuesta generada con documentos en {llm_latency:.2f}ms.")
                
            return {"generation": generation, "llm_latency_ms": llm_latency, "logs": logs}

        def node_rewrite_query(state: AgentState):
            query = state["query"]
            logs = list(state.get("logs", []))
            ...
            logs.append(f"Re-embedding generado en {emb_latency:.2f}ms")
